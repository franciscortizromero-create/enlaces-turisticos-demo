    ...
    <div className="card flex flex-col">
      <div className="relative aspect-video overflow-hidden">
        {offer.image_url ? (
          ...
          <div className="h-full bg-gradient-to-br from-navy-700 to-navy-900 flex items-center justify-center text-5xl">🌍</div>
        )}
        {offer.featured && (
          <div className="absolute top-3 left-3 rounded-full bg-gold-400 px-2.5 py-1 text-xs font-bold text-navy-900">⭐ Popular</div>
        )}
      </div>
      <div className="flex flex-col flex-1 p-5">
        <div className="text-xs text-slate-400 mb-1">{offer.city} · {offer.hotel_category} · {offer.duration}</div>
        <h3 className="font-semibold text-navy-900 leading-snug mb-2">{offer.title}</h3>
        {offer.summary && <p className="text-xs text-slate-500 mb-3">{offer.summary}</p>}
...
export async function SectionPage({ section, title, subtitle, heroImg, tag, emptyIcon }: SectionPageProps) {
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <div className="relative h-72 sm:h-96 overflow-hidden">
        <Image src={heroImg} alt={title} fill className="object-cover" sizes="100vw" priority />
        <div className="absolute inset-0 bg-gradient-to-b from-black/50 via-black/40 to-black/70" />
        <div className="absolute inset-0 flex items-center justify-center text-center px-6">
          <div>
            <span className="inline-block rounded-full border border-white/30 bg-white/10 px-4 py-1.5 text-xs font-semibold text-white backdrop-blur mb-4">{tag}</span>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-12">
        {offers.length === 0 ? (
          <div className="text-center py-20 text-slate-400">
            ...
            <p className="text-lg mb-2">Próximamente nuevos paquetes</p>
            <p className="text-sm">Contáctanos para opciones personalizadas</p>
          </div>
        ) : (
